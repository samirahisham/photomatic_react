    ...
    render(){
      ...
      return (
        <>
          ...
          <h1 className="display-4 text-left mt-2 text-dark" style={{marginLeft:100, opacity:1}}>{event?event.title.toUpperCase():null}</h1>
          <p className="lead text-left text-muted" style={{marginLeft:100, marginTop:-25}}>Location: {event?event.location:null} | Date: {event?event.date:null} at {event?event.time:null}</p>
          <p className="text-left text-monospace text-primary" style={{marginLeft:100, marginTop:-20}}>{event?event.description:null}</p>
          </div>
          <div className="row justify-content-end  mt-4 mb-4 mr-5">
              <ButtonToolbar>
                  <Button  variant="outline-primary mr-5" size="lg" onClick={() => this.setUploadShow(true)}>
                  Upload Photos
                  </Button>
          <UploadForm
          ...
          <ButtonToolbar>
          <Button variant="outline-warning" size="lg" onClick={() => this.setShareShow(true)}>
              Share Event's Album
          </Button>
